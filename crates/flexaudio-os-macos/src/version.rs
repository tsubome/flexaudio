//! macOS バージョンゲート: Process Tap は **macOS 14.4+** 必須。
//!
//! `MacSystemBackend` / `MacProcessBackend` の `start` が tap 生成へ進む前にこの
//! チェックを通し、**14.4 未満なら [`Error::UnsupportedOsVersion`]** を返す。
//! これが無いと古い OS では `AudioHardwareCreateProcessTap` が raw な `OSStatus` を
//! 返し、[`map_os_status`](crate::common::map_os_status) 経由で `Error::Backend` に
//! 化けてしまう（= Windows の `UnsupportedOsVersion` 経路と非対称になる。監査 P1-5）。
//! 型でゲートすることで「機能を満たさない OS バージョン」を 3 OS で同じ
//! `Error::UnsupportedOsVersion` に揃える。
//!
//! # 取得方法
//! `objc2-foundation` の `NSProcessInfo` 機能フィーチャを増やさずに済むよう、Foundation の
...
//! `NSInteger` 3 連）はレイアウト一致のローカルミラー [`NSOperatingSystemVersion`] へ
//! 受ける（`Encode`/`RefEncode` を実装して構造体戻り値に対応）。
...
/// Process Tap がサポートされる最小 macOS バージョン（メジャー）。
const MIN_MAJOR: i64 = 14;
/// Process Tap がサポートされる最小 macOS バージョン（マイナー）。`14.4`。
const MIN_MINOR: i64 = 4;

/// Foundation の `NSOperatingSystemVersion` とレイアウト一致のローカルミラー。
///
/// 3 つの `NSInteger`（64bit ターゲットでは `isize`/`i64`）から成る `#[repr(C)]` 構造体。
/// `msg_send!` で `operatingSystemVersion` の構造体戻り値を受けるため `Encode`/`RefEncode`
/// を実装する（Foundation 側の `NSOperatingSystemVersion` と同じく無名 struct エンコード）。
#[repr(C)]
...
/// `(major, minor)` が Process Tap の最小要件（14.4）を満たすか判定する純関数。
///
/// patch は要件に関与しない（Apple は 14.4 で導入と告知。14.4.x はすべて OK）。
/// テスト可能な純ロジックとして OS 呼び出しから切り離す。
...
///
/// `[[NSProcessInfo processInfo] operatingSystemVersion]` を直接送って読む。Foundation は
/// 常にリンクされており `NSProcessInfo` クラスは実行時に必ず存在する。
fn current_os_version() -> (i64, i64) {
    // SAFETY: `NSProcessInfo` クラスは Foundation に常在。`processInfo` はシングルトンを
    // 返す（autoreleased、ここでは即座に operatingSystemVersion を送るだけなので保持不要）。
    // `operatingSystemVersion` は NSOperatingSystemVersion を返すゼロ引数セレクタで、
    // レイアウト一致のローカルミラーで受ける。
    unsafe {
...
///
/// 各バックエンドの `start` が tap 生成（CoreAudio 呼び出し）へ進む前に呼ぶ。これにより
/// 古い OS の失敗が raw `OSStatus`→`Error::Backend` ではなく型付きの
/// `UnsupportedOsVersion` になり、Windows と対称になる。
