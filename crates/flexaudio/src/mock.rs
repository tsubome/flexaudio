...
/// `start()` または `stop()` で **panic する** テスト専用バックエンド。
///
/// 任意 OS バックエンドの start/stop は flexaudio から見れば外部コードであり、
/// （契約違反だが）panic しうる。その panic が `SharedState.backend`（start/stop を
/// 跨いでロックされる `Mutex`）を poison させ、取り込み/ウォッチドッグスレッドが
/// 次にそのロックを取った瞬間に連鎖 panic で無言死する——という回帰を防ぐための
/// バックエンドである。[`Stream`](crate::Stream) はこの panic を
/// [`Error::Backend`](flexaudio_core::types::Error::Backend) /
/// [`Event::Error`](flexaudio_core::types::Event::Error) として表に出し、他スレッドを
/// 連鎖 panic させないことを保証する（`stream.rs` の panic 回帰テスト参照）。
///
/// - [`PanicMode::Start`]: 最初の `start()` で必ず panic する。
...
/// **ウォッチドッグによる再オープン（2 回目以降の `start()`）で panic する** テスト
/// 専用バックエンド。
///
/// ウォッチドッグスレッド内で backend が panic した場合に、`SharedState.backend`
/// mutex を poison させて取り込み/ウォッチドッグスレッドを連鎖無言死させず、
